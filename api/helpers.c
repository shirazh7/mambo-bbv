...
static inline int emit_a64_add_sub_shift(mambo_context *ctx, int rd, int rn, int rm,
                                         unsigned int shift_type, unsigned int shift)
{
  if (shift < 0 || shift > 63 || shift_type > ASR)
    return -1;
  int op = (rm < 0);
  rm = abs(rm);
...
int __emit_branch_cond(inst_set inst_type, void *write, uintptr_t target, mambo_cond cond, bool link)
{
  ...
  case ARM_INST:
    if (target & THUMB)
      return -1;
    diff -= 8;
    if (diff < -33554432 || diff > 33554428)
      return -1;
...
// push to the stack to make a safe function call
...
int emit_safe_fcall_static_args(mambo_context *ctx, void *fptr, int argno, ...)
{
  ...
  if (argno > MAX_FCALL_ARGS || argno < 0)
    return -1;
  if (argno > 0)
  {
...
#endif
    if (offset < -SHIFTED_ADD_SUB_MAX || offset > SHIFTED_ADD_SUB_MAX)
      return -1;
